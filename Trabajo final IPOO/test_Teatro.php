...
$theater = new Teatro_Instalacion(); 
do {
    ...
    if ($variable == 1) {
        ...
        echo "Ingrese LA hora de Inicio (en horas): \n";
        $hora[0] = trim(fgets(STDIN));
        echo "Ingrese LOS MINUTOS de inicio: \n";
        ...
        $rta = $theater->solapamiento($hora, $duracion); 
        if (!$rta) {
            $abmCine->agregarFuncion($cine1); 
        }else{
            echo "SOLAPAMIENTO DETECTADO IMPOSIBLE CARGAR FUNCION"; 
        }
        ...
        $rta = $theater->solapamiento($hora, $duracion); 
        if ($rta) {
            echo "\nSOLAPAMIENTO DETECTADO IMPOSIBLE CARGAR FUNCION\n"; 
        }else{
        ...
        $funciones = new Teatro_Instalacion(); 
        $nuevasFunciones = $funciones->getFunciones(); 
        if (count($nuevasFunciones) == 0 ) {
            echo "No hay funciones cargadas en el teatro\n"; 
        }else{
            foreach ($nuevasFunciones as $funcion) {
                echo "------------------------------";
       ...
       if (count($nuevasFunciones) == 0 ) {
           ...
           if ($rta == 1) {
               ...
               $rta = $abmMusical->eliminarFuncion($objMusical);
               
            }elseif ($rta == 2) {
                $abmTeatro = new abmObra_Teatro(); 
                $objTeatro = $abmTeatro->seleccionFuncion($id); 
                $rta = $abmTeatro->eliminarFuncion($objTeatro); 
               
            }elseif ($rta == 3) {
                $abmCine = new abmCine(); 
                $objCine = $abmCine->seleccionFuncion($id); 
               $rta = $abmCine->eliminarFuncion($objCine); 
              
        }
    }
    }else if($variable == 10){
        echo "Ingrese el Id del teatro (No se pueden repetir): \n"; 
        $id = trim(fgets(STDIN)); 
        ...
        $abmTeatroInstalacion = new abmTeatro();
        $abmTeatroInstalacion->agregarTeatro($datosTeatroInsta); 
